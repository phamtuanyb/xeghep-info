# CLAUDE.md — Nền tảng Website Xe Ghép (White-label Multi-tenant SaaS)

> **Tài liệu hướng dẫn build cho Claude Code.** File này là nguồn chân lý (single source of truth) để AI code toàn bộ hệ thống đúng theo PRD. Mọi quyết định kỹ thuật mâu thuẫn với file này đều SAI.
>
...
1. Đặt file này tên `CLAUDE.md` ở **gốc repository**. Claude Code tự đọc khi mở session trong thư mục dự án.
2. Khi dự án lớn lên, tách chi tiết ra `.claude/rules/*.md` và import vào đây bằng cú pháp `@path/to/file.md`. Ví dụ: `@.claude/rules/multi-tenant.md`.
...
5. Khi cần làm rõ yêu cầu nghiệp vụ, đọc lại `PRD-Website-Xe-Ghep.docx` (tài liệu nghiệp vụ gốc). File `CLAUDE.md` này là bản dịch PRD sang hướng dẫn code.
...
## 14. Lộ trình build (theo thứ tự — Claude Code làm tuần tự)
